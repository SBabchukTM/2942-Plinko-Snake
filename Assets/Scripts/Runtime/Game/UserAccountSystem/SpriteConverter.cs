...
namespace Runtime.Game.UserAccountSystem
{
    ...
    {
        public string ToString(Sprite sprite, int maxSize)
        {
            var readableTexture = ChangeTexture(sprite.texture, maxSize);
            return Convert.ToBase64String(readableTexture.EncodeToPNG());
        }

        public Sprite ToSprite(string avatarBase64)
        {
        ...
        private Texture2D ChangeTexture(Texture2D source, int maxSize)
        {
